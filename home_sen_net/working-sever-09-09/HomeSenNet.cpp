...
void HSN::I_Have(int Ihave , ...){//declare sensors/node - initialize: Peripherals matrix; Sensor Pins
	...
	for ( int x = 0; x < Ihave; x++ ) // iterate over all arguments and initialize peripherals matrix
    {
		...
		if (peri==0){
		...
		}else if(peri==3){//node 3 - blue led strip on pin 3
			...
			bool BLUEsuccess = SetPinFrequencySafe(sensor_pin_map[peri], frequency);  //sets the frequency for the specified pin
...
void HSN::ExecuteCommand(RF24 RADIO){
	...
	if ((sensor == 2)&& (value <256)){ //2 - Green Led Bookshelf - pin 5
		analogWrite(sensor_pin_map[sensor],255-value);
		sensor_state[sensor]=value;
		//Serial.print(sensor_map[sensor]);Serial.print(" value: "); Serial.println(value);
		ProvideFeedback(RADIO);
	...
	if ((sensor == 3)&& (value <256)){ //3 - Blue Led Bookshelf - pin 3
		//analogWrite(sensor_pin_map[sensor],255-value);
		pwmWrite(sensor_pin_map[sensor],255-value); //need to use this function instead of analogWrite to be able to usecorrect frecuency
		sensor_state[sensor]=value;
		Serial.print(sensor_map[sensor]);Serial.print(" value: "); Serial.println(value);
		ProvideFeedback(RADIO);
